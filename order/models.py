...
class Ordered(BaseModel):
    ...
    purchase_date = models.DateTimeField(verbose_name=_("Date"), help_text=_("Date of Purchase"))
    STATUS_CHOICES = (
        ('processing', 'PROCESS'),
        ('canceled', 'CANCEL'),
        ('paid', 'PAID'),
...
class OrderedProduct(BaseModel):
    ...
    order_id = models.ForeignKey(Ordered, verbose_name=_("Order"), help_text=_("Order of  SubOrder"),
                                 on_delete=models.CASCADE)
    quantity = models.IntegerField(verbose_name=_("Quantity"), help_text=_("Quantity of  SubOrder"))
